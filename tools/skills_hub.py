...
class SkillsShSource(SkillSource):
    """Discover skills via skills.sh and fetch them from the underlying GitHub repo."""

    SEARCH_URL = "https://skills.sh/api/search"

    def __init__(self, auth: GitHubAuth):
        self.auth = auth
        # Use the existing GitHub source implementation for actual fetch/inspect work.
        self.github = GitHubSource(auth=auth, extra_taps=[])
    ...
    def trust_level_for(self, identifier: str) -> str:
        ...
        if len(parts) >= 2:
            return self.github.trust_level_for(f"{parts[0]}/{parts[1]}/placeholder")
        return "community"

    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        cache_key = f"skills_sh_search_{hashlib.md5(query.encode()).hexdigest()}"
        ...
        for entry in skills[:limit]:
            ...
            wrapped = self._wrap_identifier(identifier)
            meta = self.inspect(wrapped)
            if meta:
                results.append(meta)
                continue

            name = str(entry.get("name") or identifier.rsplit("/", 1)[-1])
            repo = str(entry.get("source") or "/".join(identifier.split("/", 2)[:2]))
            installs = entry.get("installs")
            description = f"skills.sh skill from {repo}"
            ...
            results.append(
                SkillMeta(
                    ...
                    trust_level=self.trust_level_for(identifier),
                    repo=repo,
                    path=identifier.split("/", 2)[-1] if "/" in identifier else identifier,
                )
            )
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for candidate in self._candidate_identifiers(canonical):
            ...
            if bundle:
                ...
                bundle.trust_level = self.trust_level_for(canonical)
                return bundle
        return None
...
class WellKnownSkillSource(SkillSource):
    """Read skills from a site exposing /.well-known/skills/index.json."""
    ...
    def _query_to_index_url(self, query: str) -> Optional[str]:
        ...
        if raw.endswith("/index.json"):
            return raw
        if f"{self.BASE_PATH}/" in raw:
            return raw.split(f"{self.BASE_PATH}/", 1)[0] + f"{self.BASE_PATH}/index.json"
        return raw.rstrip("/") + f"{self.BASE_PATH}/index.json"
    ...
    def _fetch_text(url: str) -> Optional[str]:
        try:
            ...
            logger.debug("Well-known skills fetch failed: %s", e)
        return None
