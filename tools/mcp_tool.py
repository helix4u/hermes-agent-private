...
def _select_utility_schemas(
    ...
    resources_enabled = _parse_boolish(
        tools_filter.get("resources"),
        default=True,
    )
    prompts_enabled = _parse_boolish(
        tools_filter.get("prompts"),
        default=True,
    )
...
def _existing_tool_names() -> List[str]:
    ...
    for _sname, server in _servers.items():
        if getattr(server, "_registered_tool_names", None):
...
async def _discover_and_register_server(name: str, config: dict) -> List[str]:
    ...
    # Register MCP Resources & Prompts utility tools when enabled in config
    # and supported by the connected server session.
    _handler_factories = {
    ...
    for name, cfg in configured.items():
        ...
        if server and server.session is not None:
            result.append({
                ...
                if getattr(server, "_registered_tool_names", None) is not None
